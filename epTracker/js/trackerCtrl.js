...
app.controller("trackerCtrl", function($scope) 
{
	...
	$scope.showDetail=function($event) {
		...
		if($scope.amountGoingToPaid!=0) {
			$scope.msg='Total amount paid:' +$scope.amountGoingToPaid.toFixed(2);
			...
			if($scope.summary[x].Year==$scope.year) {
				if(recordType=='Income') {
					...
					if($scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text])
						$scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text]= parseFloat($scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text]) + parseFloat(amount);
					else
						$scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text]=parseFloat(amount);
						alert($scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text]);
				}
					...
					if($scope.summary[x].Month[month-1].Transfer[e.options[e.selectedIndex].text])
						$scope.summary[x].Month[month-1].Transfer[e.options[e.selectedIndex].text]=parseFloat($scope.summary[x].Month[month-1].Transfer[e.options[e.selectedIndex].text]) + parseFloat(amount);
					else
