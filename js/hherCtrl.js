...
app.controller("hherCtrl", function($scope) 
{
	...
	$scope.saveItem=function(){
		if($scope.newItem=='') {
			$scope.msg='Please input an item!';
			return;
		}
	...
	$scope.change=function(){
		...
		if($scope.type=='Transfer')
			document.getElementById('containerToAccount').style.display='block';
		else
			document.getElementById('containerToAccount').style.display='none';
	},
	...
	$scope.addDetail=function(){
		...
		rd= new Date($scope.year+'/'+$scope.month+'/'+$scope.day);
		...
		for(x in $scope.summary) {
			if($scope.summary[x].Year==$scope.year) {
				if(recordType=='Income') {
					$scope.summary[x].Income+=amount;
					$scope.summary[x].Month[month-1].Income.total+=amount;
					if($scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text])
						$scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text]+=amount;
					else
						$scope.summary[x].Month[month-1].Income[e.options[e.selectedIndex].text]=amount;
				}
				else if(recordType=='Transfer') {
					$scope.summary[x].Transfer+=amount;
					$scope.summary[x].Month[month-1].Transfer.total+=amount;
					if($scope.summary[x].Month[month-1].Transfer[e.options[e.selectedIndex].text])
						...
						$scope.summary[x].Month[month-1].Transfer[e.options[e.selectedIndex].text]=amount;
					
				}
				else {
					$scope.summary[x].Expense+=amount;
					$scope.summary[x].Month[month-1].Expense.total+=amount;
					if($scope.summary[x].Month[month-1].Expense[e.options[e.selectedIndex].text])
						$scope.summary[x].Month[month-1].Expense[e.options[e.selectedIndex].text]+=amount;
					else
						$scope.summary[x].Month[month-1].Expense[e.options[e.selectedIndex].text]=amount;
				}
		...
		for(i in $scope.details) {
			if($scope.details[i].Date==date) {
				if($scope.details[i].Paid==0) {
					...
					$scope.amountGoingToPaid+=$scope.details[i].Amount;
				}
				else {
	...
	$scope.init=function(){
		...
		$scope.title.push(newTitle);
		newTitle={name:'Paid',hidden:true};
		$scope.title.push(newTitle);
		var newSummary=
		{
			Year:year,
